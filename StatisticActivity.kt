...
class StatisticActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun selectData(
            ...
            startDate: java.util.Date,//java.util
            endDate: java.util.Date, //java.util
            income: Boolean,
            ...
            //Toast.makeText(this,""+start.time.toString(),Toast.LENGTH_SHORT).show()
        ...
        //每日支出的数据包
        fun getDayPay(needAccount: String, needDate: java.util.Date):List<Record>{
            return selectData(
        ...
        //每日收入的数据包
        fun getDayIncome(needAccount: String, needDate: java.util.Date):List<Record>{
            return selectData(
                needAccount = needAccount,
                startDate = needDate,
                endDate = needDate,
        ...
        //每日收支
        fun getDayBalance(needAccount: String, needDate: java.util.Date):Float{
        ...
        fun getMonthPay(needAccount: String, needCalendar: Calendar):List<Record> {
            val startDate = java.util.Date(needCalendar.timeInMillis)
            needCalendar.add(Calendar.MONTH, -1)
            val endDate = java.util.Date(needCalendar.timeInMillis)
            return selectData(
                needAccount = needAccount,
                startDate = startDate,
                endDate = endDate,
        ...
        button_s_add.setOnClickListener{
            ...
            Log.d("select", ""+searchCategory.toString())
            Log.d("select", ""+searchSubCategory.toString())
        }
        ...
        //下一步按钮，应该跳转至图表,临时用来debug
        s1_next.setOnClickListener {
            //Toast.makeText(this, "debug", Toast.LENGTH_LONG).show()
            val Account = "我"
            val debugCategory:MutableList<String> = mutableListOf("餐饮")
            val debugSubCategory1:MutableList<String> = mutableListOf("早餐")
            val debugSubCategory2:MutableList<String> = mutableListOf("早餐","午餐")
            val debugSubCategory3:MutableList<String> = mutableListOf("早餐","午餐","晚餐")
            val num=0
            //Toast.makeText(this,"Category "+debugSubCategory2[0]+debugSubCategory2[1],Toast.LENGTH_SHORT).show()
            val debugList=selectData(Account,startDate = startDate,endDate = endDate,income = true)
            //val debugList2 = Record(1,"",Date(startDate.time), Time(100),"lsd","1111",
                //"1111.2","我",0F,"666",true,"steam","games")
            //Toast.makeText(this,endDate.time.toString(),Toast.LENGTH_LONG).show()
            Toast.makeText(this,debugList.isEmpty().toString(),Toast.LENGTH_LONG).show()
            //Toast.makeText(this,""+,Toast.LENGTH_SHORT).show()
        }

    }
    private val timeend:MutableList<Int> = mutableListOf()
    private val timestart:MutableList<Int> = mutableListOf()
    //timestart[0]:Year
    //timestart[1]:month
    //timestart[1]:day
    ...
    private var startDate = java.util.Date(startCalendar.timeInMillis)//<--------------------------开始时间和结束时间的java.util.Date格式
    private var endDate = java.util.Date(endCalendar.timeInMillis)


    //日历组件
    @SuppressLint("SetTextI18n")
    fun SelectEndTime(view: View) {
        Toast.makeText(this, "选择结束时间", Toast.LENGTH_LONG).show()
        ...
        val listener =
            OnDateSetListener { arg0, year, month, day ->
                ...
                timeend.add(day)
                Log.d("ggg", ""+timeend[2])
                endCalendar.set(timeend[0],timeend[1],timeend[2],23,59,59)
                endDate = java.util.Date(endCalendar.timeInMillis)
            }
    ...
    fun SelectStartTime(view: View) {
        Log.d("123", "23324")
        Toast.makeText(this, "选择开始时间", Toast.LENGTH_LONG).show()
        var cal=Calendar.getInstance()
        var year=cal.get(Calendar.YEAR)       //获取年月日时分秒
        var month=cal.get(Calendar.MONTH)   //获取到的月份是从0开始计数
        var day=cal.get(Calendar.DAY_OF_MONTH)
        ...
        val listener =
            OnDateSetListener { arg0, year, month, day ->
                ...
                timestart.add(month)
                timestart.add(day)
                Log.d("123", ""+timestart[0]+"--"+timestart[1]+"--"+timestart[2])
                startCalendar.set(timestart[0],timestart[1],timestart[2],0,0,0)
                startDate = java.util.Date(startCalendar.timeInMillis)
            }
        val dialog = DatePickerDialog(this, 0, listener, year, month, day) //后边三个参数为显示dialog时默认的日期，月份从0开始，0-11对应1-12个月
        dialog.show()
    }
