...
interface UserDAO {
    ...
    //分类统计成员一级二级选项的数据包
    @Query("SELECT * FROM Record WHERE account = (:SelectAccount) And date > :StartDate AND date <:EndDate and member in (:SelectMember)AND category in(:SelectCategory)And subcategory IN (:SelectSubCategory) and item in (:SelectItem) and merchant in(:SelectMerchant)AND income=(:SelectIncome)and type in(:SelectType)")
